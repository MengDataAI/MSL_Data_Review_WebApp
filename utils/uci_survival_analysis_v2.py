...
##################################################################################################
# the input is first_year_df dataframe, and the output is forest plot
#
##################################################################################################
...
##################################################################################################
# the input is first_year_df dataframe, and the output is KM plot between moderate and low groups
#
##################################################################################################
def plot_km_moderate_low(first_year_df):
    ...
    fig.update_layout(
        ...
        xaxis=dict(range=[0, 395], dtick=30, showgrid=False, showline=True, linewidth=2, linecolor='gray', tickfont=dict(size=16), titlefont=dict(size=18)),
...
##################################################################################################
# the input is first_year_df dataframe, and the output is KM plot between living and deceased groups
#
##################################################################################################
...
##################################################################################################
# the input is first_year_df dataframe, and the output is a risk table for KM analysis between moderate and low groups
#
##################################################################################################
def get_km_moderate_low_risk_table(first_year_df, time_points=None):
    # Prepare data for survival analysis (same logic as plot_km_moderate_low)
    df = first_year_df[['PID', 'ACCESSIONID', 'RESULT_POST_TRANSPLANT', 'DONOR_TYPE', 'RES_RESULT_NUMERIC', 'ON_PROTOCOL', 'PROTOCOL_TESTING_MONTH', 'TRANSPLANT_AGE', 'SEX']].copy()
    df['TRANSPLANT_AGE'] = df['TRANSPLANT_AGE'].astype(int)
    df = df[(df['ON_PROTOCOL'] == 'Y') & (df['PROTOCOL_TESTING_MONTH'].notna())]
    df = df.sort_values(by=['PID', 'RESULT_POST_TRANSPLANT'])
    ...
    df['TIME_TO_EVENT'] = df['TIME_TO_EVENT'].astype(int)
    df['EVENT_OCCURRED'] = df['EVENT_OCCURRED'].astype(int)
    km_df = df[['PID', 'TIME_TO_EVENT', 'EVENT_OCCURRED', 'RISK_GROUP']].drop_duplicates().sort_values(by=['PID', 'TIME_TO_EVENT'])
    ...
    # Default time points: every 30 days up to 390 (like your image)
    if time_points is None:
        time_points = list(range(0, 391, 30))
    
    risk_table = {}
    for group, label, kmf, group_df in [('MR', 'Moderate Risk', kmf_mr, group_MR), ('LR', 'Low Risk', kmf_lr, group_LR)]:
        at_risk = []
        censored = []
        events = []
        
        for t in time_points:
            # Get the number at risk at time t (this is what KM uses)
            n_at_risk = kmf.survival_function_.index[kmf.survival_function_.index >= t].shape[0] if len(kmf.survival_function_) > 0 else 0
            
            # Count events and censored in the interval [0, t]
            if t == 0:
                n_events = 0
                n_censored = 0
            else:
                # Events: patients whose event time <= t and EVENT_OCCURRED==1
                n_events = ((group_df['TIME_TO_EVENT'] <= t) & (group_df['EVENT_OCCURRED'] == 1)).sum()
                # Censored: patients whose censoring time <= t and EVENT_OCCURRED==0
                n_censored = ((group_df['TIME_TO_EVENT'] <= t) & (group_df['EVENT_OCCURRED'] == 0)).sum()
    ...
    # Build DataFrame: MultiIndex (Group, Metric) x Time
    arrays = [[], []]
    data = []
    for group in ['Moderate Risk', 'Low Risk']:
        for metric in ['At risk', 'Censored', 'Events']:
            arrays[0].append(group)
            arrays[1].append(metric)
            data.append(risk_table[group][metric])
    index = pd.MultiIndex.from_tuples(list(zip(*arrays)), names=['Group', 'Metric'])
    risk_df = pd.DataFrame(data, index=index, columns=time_points)
    return risk_df


##################################################################################################
# the input is first_year_df dataframe, and the output is a risk table for KM analysis between living and deceased groups
#
##################################################################################################
def get_km_donor_type_risk_table(first_year_df, time_points=None):
    # Prepare data for survival analysis (same logic as plot_km_donor_type)
    df = first_year_df[['PID', 'ACCESSIONID', 'RESULT_POST_TRANSPLANT', 'DONOR_TYPE', 'RES_RESULT_NUMERIC', 'ON_PROTOCOL', 'PROTOCOL_TESTING_MONTH', 'TRANSPLANT_AGE', 'SEX']].copy()
    df['TRANSPLANT_AGE'] = df['TRANSPLANT_AGE'].astype(int)
    df = df[(df['ON_PROTOCOL'] == 'Y') & (df['PROTOCOL_TESTING_MONTH'].notna())]
    df = df.sort_values(by=['PID', 'RESULT_POST_TRANSPLANT'])
    ...
    df['TIME_TO_EVENT'] = df['TIME_TO_EVENT'].astype(int)
    df['EVENT_OCCURRED'] = df['EVENT_OCCURRED'].astype(int)
    km_df = df[['PID', 'TIME_TO_EVENT', 'EVENT_OCCURRED', 'DONOR_TYPE']].drop_duplicates().sort_values(by=['PID', 'TIME_TO_EVENT'])
    ...
    risk_table = {}
    for group, label, kmf, group_df in [('Deceased', 'Deceased Donor', kmf_DECEASED, group_DECEASED), ('Living', 'Living Donor', kmf_LIVING, group_LIVING)]:
        at_risk = []
        censored = []
        events = []
    ...
    arrays = [[], []]
    data = []
    for group in ['Deceased Donor', 'Living Donor']:
        for metric in ['At risk', 'Censored', 'Events']:
            arrays[0].append(group)
            arrays[1].append(metric)
            data.append(risk_table[group][metric])
...
# test 
#plot_km_moderate_low(first_year_df)
#get_km_moderate_low_risk_table(first_year_df)
plot_km_donor_type(first_year_df)
#get_km_donor_type_summary_table(first_year_df)
#plot_forest_cox(first_year_df)