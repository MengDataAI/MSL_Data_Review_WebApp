...
def plot_low_risk_violin(first_year_df):
    ...
    for protocol_month in protocol_order:
        df_subset = low_risk_df[low_risk_df['PROTOCOL_TESTING_MONTH'] == protocol_month]
        if not df_subset.empty:
            fig.add_trace(go.Violin(
                y=df_subset['RES_RESULT_NUMERIC'],
                x=[protocol_month] * len(df_subset),
                name='Low Risk',
                line_color='#2E8B57',  # Sea Green for low risk (Nature/Science standard)
                spanmode='hard',
...
def plot_low_risk_lines(first_year_df):
    ...
    # Add summary statistics as annotations with better styling (right-aligned)
    fig.add_annotation(
