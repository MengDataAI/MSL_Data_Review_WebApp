...
##################################################################################################
# the input is first_year_df dataframe, and the output is plotly line for high risk > 253% 
#
##################################################################################################
def plot_high_risk_253(first_year_df):
    ...
    rcv_plot_high['RCV'] = rcv_plot_high['RELATIVE_BETWEEN_TEST'].apply(
    ...
    for i, patient_id in enumerate(high_change_patients_high_filtered_df['PID'].unique()):
        patient_data = high_change_patients_high_filtered_df[high_change_patients_high_filtered_df['PID'] == patient_id]
    ...
    fig.update_layout(
        title=dict(
            text='<b>High Risk Patients with Sequential AlloSure Score RCV >253%</b>',
            x=0.5,
            xanchor='center',
            font=dict(size=20)
        ),
...
# test
plot_high_risk_253(first_year_df)
